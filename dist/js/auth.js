...
const Auth = {
    ...
    async signUp(email, password, userData = {}) {
      try {
        ...
        if (window.Toast) {
          window.Toast.success('Registration successful! You can now log in.');
        }
        return { success: true, message: 'Registration successful! You can now log in.', user: data.user };
      } catch (err) {
        console.error('❌ SignUp Error:', err.message);
        if (window.Toast) {
          window.Toast.error(err.message || 'Failed to create account');
        }
    ...
    async signIn(email, password) {
      try {
        ...
        if (window.Toast) {
          window.Toast.error(err.message || 'Failed to sign in');
        }
